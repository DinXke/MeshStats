...
MeshManager publishes statistics about a radio network. The data itself is not
...
- Generated as `"mm_" + secrets.token_urlsafe(32)` — 256 bits.
...
|---|---|---|
| Every logged-in admin form | `mm_session` | with the session (12 h) |
...
`MM_MAX_BODY_BYTES`, default 2 MB, on every route and method.
...
right. `ratelimit.client_ip()` counts `MM_TRUSTED_PROXY_HOPS` entries in from
...
Set `MM_TRUSTED_PROXY_HOPS` to the number of proxies you actually run. Too high
...
- **The topic names the publisher.** `meshmanager/<node_hex>/stats` — the node that
  sent the message.
...
account keeps `topic write meshmanager/#` so nothing breaks mid-migration — remove
...
(`firmware/examples/simple_repeater/MeshManagerNet.cpp`). That verifies an image is a
...
- **The `"via"` field is safe because the server ignores it.** Relayed messages
  ...
  (`MeshManagerNet.cpp` ~line 2588). `mqtt_ingest.py` never reads it: publisher
...
- [ ] Set `MM_TRUSTED_PROXY_HOPS` to the number of proxies actually in front of
...
- [ ] One broker account per node (`mosquitto/add-node-user.sh`), then drop
      `topic write meshmanager/#` from the shared account in `mosquitto/acl`
