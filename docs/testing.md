...
| `test_alerts.py` | 27 | `alerts`, the `alert` topic, the acked flow | Telemetry is polling, an alert is a trap: the subject versus the relayer, repeats, a clock from 2024, and why nothing deletes an alert |
...
| `test_rechten.py` | 31 | `app/rbac.py`, `app/audit.py`, the migration | Three ways a permission model breaks without raising anything: too wide, too narrow (the owner locked out), and forgotten on a route |
