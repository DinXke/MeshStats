...
| [`rooms.md`](rooms.md) | Een MeshUptime-room-server-node beheren over zijn eigen HTTP-API: rooms, virtuele sensor-nodes en de notifier-bot lezen en beheren, de join-/contact-QR en link, per-sleutel-toegangsbeheer (ACL), node-centrisch kanaalbeheer inclusief SNMP-monitors met een preset-OID-bibliotheek, een kiezer van ontdekte contacten, handmatige adverts, serverzijde backups, en hoe losse room-/sensor-node-/bot-entries teruggekoppeld worden aan hun fysieke node |
