...
(function () {
  ...
  var DICT = {
    nl: {
      ...
      // --- live pakketkaart ---
      "filtermesh.title": "Pakketfilter in dit mesh",
      "filtermesh.dropped": "geweerd",
      "filtermesh.passed": "doorgelaten",
      "filtermesh.exempt": "langs het filter via de access list",
      "filtermesh.scopeOn": "Filter aan bij",
      "filtermesh.scopeOf": "van de",
      "filtermesh.scopeNodes": "repeaters op deze site",
      "filtermesh.scopeOff": "Meldt uitdrukkelijk geen filter:",
      "filtermesh.scopeUnknown": "Nooit iets over een filter gemeld:",
      "filtermesh.scopeUnknownTail": " — dat is geen bewering dat daar niets aanstaat.",
      "filtermesh.reason": "Waarop geweerd",
      "filtermesh.count": "Aantal",
      "filtermesh.period": "Elke node telt sinds zijn eigen laatste herstart, dus dit is een optelsom over ongelijke periodes en geen momentopname van een vast venster.",
      ...
      "arch.help_syntax": "Clausules met een spatie ertussen moeten allemaal gelden. veld:waarde zoekt exact, -type:ACK sluit uit, type:(ADVERT OR ACK) is een van beide, aanhalingstekens voor een waarde met spaties. Een los woord zoekt in namen, sleutels, type, bereik en land.",
      "arch.help_wildcards": "Een sterretje staat voor “wat dan ook” en mag overal in een tekstveld staan: name:BE-HSS* begint ermee, name:*circuit eindigt erop, name:*circuit* bevat het. Een los woord zoekt altijd al op bevatten; het sterretje is er om dat binnen één veld te doen. Getalvelden nemen geen sterretje — daar zijn groter en kleiner dan voor.",
      ...
      "arch.f_scope": "Bereik",
      "arch.f_filter": "Doorgelaten of geweerd",
      "arch.f_reden": "Reden van weren",
      "arch.f_region": "Regio",
      ...
      "reason.hash": "padhash te klein",
      "reason.kanaal": "geblokkeerd kanaal",
      "reason.misvormd": "misvormde groepstekst",
      ...
      "metric.wifi_rssi": "WiFi-signaal",
      "metric_hint.wifi_rssi": "Het signaal van de WiFi-verbinding, niet van de LoRa-radio. Deze node wordt over IP beheerd; zakt dit weg, dan verdwijnt die beheerweg en blijft alleen het mesh over.",
    ...
    en: {
      ...
      "filtermesh.title": "Packet filter across this mesh",
      "filtermesh.dropped": "refused",
      "filtermesh.passed": "forwarded",
      "filtermesh.exempt": "past the filter via the access list",
      "filtermesh.scopeOn": "Filter on at",
      "filtermesh.scopeOf": "of the",
      "filtermesh.scopeNodes": "repeaters on this site",
      "filtermesh.scopeOff": "Explicitly report no filter:",
      "filtermesh.scopeUnknown": "Never said anything about a filter:",
      "filtermesh.scopeUnknownTail": " — which is not a claim that none is running there.",
      "filtermesh.reason": "Refused on",
      "filtermesh.count": "Count",
      "filtermesh.period": "Each node counts since its own last restart, so this is a sum over unequal periods rather than a snapshot of one fixed window.",
      ...
      "arch.help_syntax": "Space-separated clauses must all hold. field:value matches exactly, -type:ACK excludes, type:(ADVERT OR ACK) is either, quotes for a value with spaces. A bare word searches names, keys, type, scope and country.",
      "arch.help_wildcards": "A star stands for “anything” and may sit anywhere in a text field: name:BE-HSS* starts with it, name:*circuit ends with it, name:*circuit* contains it. A bare word already searches on containment; the star is there to do that inside one field. Numeric fields take no star — that is what greater and less than are for.",
      ...
      "arch.f_scope": "Scope",
      "arch.f_filter": "Forwarded or refused",
      "arch.f_reden": "Reason for refusing",
      ...
      "reason.hash": "path hash too small",
      "reason.kanaal": "blocked channel",
      "reason.misvormd": "malformed group text",
      ...
      "metric.wifi_rssi": "WiFi signal",
      "metric_hint.wifi_rssi": "The WiFi link, not the LoRa radio. This node is managed over IP; if this drops away, that management path goes with it and only the mesh is left.",
