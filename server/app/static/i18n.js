...
(function () {
  ...
  var DICT = {
    nl: {
      ...
      "live.pacman": "Pacman-modus",
      "live.pacman_title": "Zelfde pakketten, zelfde tijden: de stip wordt een hongerige bol die de bolletjes van zijn eigen spoor opeet",
      ...
      // Twee blokken en niet één: de tegels zeggen HOEVEEL het filter weggooide,
      // de uitsplitsing WAAROP. Ze staan los van elkaar op de pagina omdat de
      // tweede alleen bestaat als de firmware er iets over meldt.
      "block.filter": "Pakketfilter",
      "block.filterstats": "Filter: uitsplitsing",
      "block.other": "Overig",
      ...
      // De filtertegels. Zonder deze regels valt de tegelrij terug op het
      ...
      "metric.filter_exempt": "Vrijgesteld (ACL)",
      "metric.filter_drop_hops": "Weg: te veel hops",
      "metric.filter_drop_rate": "Weg: snelheidslimiet",
      "metric.filter_drop_type": "Weg: type dicht",
      "metric.filter_drop_hash": "Weg: padhash te klein",
      "metric.filter_drop_channel": "Weg: geblokkeerd kanaal",
      "metric.filter_drop_malformed": "Weg: misvormde groepstekst",
      "metric.filter_rate_windows": "Snelheidsvensters met verkeer",
      "metric.filter_rate_capped": "Vensters waarin de limiet beet",
      "metric.request_successes": "Verzoeken gelukt",
      "metric.request_failures": "Verzoeken mislukt",
      "metric.out_path": "Uitgaand pad",

      // --- filter: uitsplitsing (repeaterpagina) ---
      "filter.trunc": "De node meldde dat deze uitsplitsing niet volledig in het bericht paste. Wat er staat klopt; het is alleen niet alles.",
      "filter.perType": "Weggegooid per pakkettype",
      "filter.type": "Type",
      "filter.total": "Totaal",
      "filter.why": "Waarop",
      "filter.ratePressure": "Druk op de snelheidslimiet",
      "filter.rateHint": "Hoe vaak de limiet werkelijk beet, tegenover hoe vaak hij de kans had. Een laag aandeel is een limiet die ruim staat; een hoog aandeel er een die structureel verkeer wegsnijdt.",
      "filter.windows": "Vensters",
      "filter.bit": "Beet",
      "filter.share": "Aandeel",
      "filter.peak": "Piek",
      "filter.limit": "Limiet",
      "filter.exempt": "Via de access list langs het filter",
      "filter.exemptHint": "Verkeer dat de regels geraakt zou hebben en er via de access list langs mocht. Staat dit hoog naast een lage ‘Doorgelaten’, dan werkt het filter vooral voor wie er toch al buiten viel.",
      "filter.count": "Aantal",
      "filter.channels": "Geblokkeerde kanalen",
      "filter.channelsHint": "Een repeater ziet geen kanaalnaam, alleen één byte: sha256(sleutel)[0]. Die byte staat onversleuteld in elk groepsbericht, dus hij verraadt hier niets wat niet al door de lucht gaat — ongeveer 1 op de 256 kanalen deelt hem met een ander.",
      "pfhelp.meshcore_filter.filter_drop_channel": "Kanaalverkeer weggegooid door de kanaallijst. Kijk goed welke kant die lijst op werkt bij deze variant. Regel: Filtert groepstekst op de kanalen in deze lijst. Welke kant de lijst op werkt -- doorlaten of blokkeren -- staat niet in de firmware-strings van deze build; controleer het met `filter channel list` op de node voordat u erop vertrouwt.",
      "pfhelp.meshcore_filter.filter_drop_hash": "Pakketten weggegooid omdat hun padhash korter was dan de ingestelde minimumlengte. Regel: Gooit pakketten weg met een padhash KLEINER dan min_bytes.",
      "pfhelp.meshcore_filter.filter_drop_hops": "Pakketten weggegooid omdat ze te veel hops hadden afgelegd. Regel: Gooit pakketten van dat type weg die MEER hops aflegden dan max_hops. 0 betekent hier GEEN limiet -- niet 'alles weg'.",
      "pfhelp.meshcore_filter.filter_drop_malformed": "Pakketten weggegooid omdat hun tekstinhoud als beschadigd herkend werd. Regel: Gooit pakketten weg waarvan de tekstinhoud als beschadigd herkend wordt.",
      "pfhelp.meshcore_filter.filter_drop_rate": "Pakketten weggegooid omdat de snelheidslimiet van hun type vol zat. Regel: Gooit pakketten van dat type weg boven <limit> stuks per <secs> seconden.",
      "pfhelp.meshcore_filter.filter_drop_type": "Pakketten weggegooid omdat hun hele pakkettype dichtstaat. Loopt dit op, dan is er geen limiet geraakt maar een deur dicht. Deze variant kan een pakkettype niet in één klap dichtzetten en telt er dus ook niets voor. Het strengste wat hier kan is `filter rate <type> 1 <secs>` of een lage hoplimiet.",
      "pfhelp.meshcore_filter.filter_dropped": "Alle weggegooide pakketten bij elkaar: de som van de droptellers per reden. Het cijfer om te zien DAT er gefilterd wordt; de reden staat in de tegels ernaast. Regel: Zet het hele filter aan of uit. Staat hij uit, dan doet GEEN ENKEL subfilter iets -- de regels blijven staan maar er wordt niets weggegooid.",
      "pfhelp.meshcore_filter.filter_exempt": "Pakketten die de regels helemaal oversloegen omdat ze vrijgesteld waren. Loopt dit op terwijl u niets vrijstelde, dan filtert deze node minder dan u denkt. Deze variant kent geen vrijstellingen en meldt de teller niet.",
      "pfhelp.meshcore_filter.filter_on": "1 zolang het filter aanstond, 0 zolang het uitstond. Als reeks, zodat 'wanneer stond dit filter aan' een antwoord heeft en geen gok op basis van wanneer er weer iets weggegooid werd. Regel: Zet het hele filter aan of uit. Staat hij uit, dan doet GEEN ENKEL subfilter iets -- de regels blijven staan maar er wordt niets weggegooid.",
      "pfhelp.meshcore_filter.filter_passed": "Pakketten die het filter langs alle regels doorliet. Alleen samen met `filter_dropped` bruikbaar: 500 weg is veel bij 600 door en weinig bij 600.000 door. Regel: Zet het hele filter aan of uit. Staat hij uit, dan doet GEEN ENKEL subfilter iets -- de regels blijven staan maar er wordt niets weggegooid.",
      "pfhelp.meshcore_filter.filter_rate_capped": "GEEN dropteller. Het aantal vensters waarin de limiet werkelijk BEET. Deel dit door `filter_rate_windows` en u hebt het cijfer waarmee u een limiet bijstelt: 12 van 4000 vensters is een limiet die af en toe een uitschieter afvlakt en verder niets doet; 3900 van 4000 is een limiet die de normale gang van zaken is geworden en dus te laag staat -- terwijl het aantal weggegooide pakketten in beide gevallen hetzelfde kan zijn. Dáárom staan deze twee naast de dropteller en niet in plaats van hem. Regel: Gooit pakketten van dat type weg boven <limit> stuks per <secs> seconden.",
      "pfhelp.meshcore_filter.filter_rate_windows": "GEEN dropteller. Het aantal tijdvensters waarin er verkeer van dit soort langskwam -- dus hoe vaak de limiet de KANS had om te bijten. De noemer van de verhouding hieronder. Regel: Gooit pakketten van dat type weg boven <limit> stuks per <secs> seconden.",
      "pfhelp.meshmanager.filter_drop_channel": "Kanaalverkeer weggegooid door de kanaallijst. Kijk goed welke kant die lijst op werkt bij deze variant. Regel: `channel add` BLOKKEERT juist dat kanaal voor groepstekst; alles wat niet op de lijst staat gaat door. Andersom dan bij de stock-variant.",
      "pfhelp.meshmanager.filter_drop_hash": "Pakketten weggegooid omdat hun padhash korter was dan de ingestelde minimumlengte. Regel: Stuurt alleen pakketten door met een padhash van minstens min_bytes byte.",
      "pfhelp.meshmanager.filter_drop_hops": "Pakketten weggegooid omdat ze te veel hops hadden afgelegd. Regel: Laat dat pakkettype nog hoogstens max_hops hops reizen. 0 betekent hier: dit type HELEMAAL niet meer doorsturen -- omgekeerd aan de stock-variant.",
      "pfhelp.meshmanager.filter_drop_malformed": "Pakketten weggegooid omdat hun tekstinhoud als beschadigd herkend werd. Regel: Gooit groepstekst met een onmogelijke structuur weg.",
      "pfhelp.meshmanager.filter_drop_rate": "Pakketten weggegooid omdat de snelheidslimiet van hun type vol zat. Regel: Gooit pakketten van dat type weg boven <limit> stuks per <secs> seconden.",
      "pfhelp.meshmanager.filter_drop_type": "Pakketten weggegooid omdat hun hele pakkettype dichtstaat. Loopt dit op, dan is er geen limiet geraakt maar een deur dicht. Regel: `off` stuurt dat pakkettype HELEMAAL niet meer door -- geen limiet maar een dichte deur. Dit is de zwaarste filterhandeling die deze firmware kent.",
      "pfhelp.meshmanager.filter_dropped": "Alle weggegooide pakketten bij elkaar: de som van de droptellers per reden. Het cijfer om te zien DAT er gefilterd wordt; de reden staat in de tegels ernaast. Regel: Zet het hele filter aan of uit. Staat hij uit, dan doet GEEN ENKEL subfilter iets. Let op: `filter on` terwijl er al een categorale regel klaarstaat, is de klik die dat verkeer stilzet.",
      "pfhelp.meshmanager.filter_exempt": "Pakketten die de regels helemaal oversloegen omdat ze vrijgesteld waren. Loopt dit op terwijl u niets vrijstelde, dan filtert deze node minder dan u denkt. Deze firmware MELDT de vrijstellingsteller, maar in deze codebase staat geen filtercommando dat de vrijstellingslijst zet. Wat de lijst vult is hier niet te onderbouwen, dus staat er geen syntax.",
      "pfhelp.meshmanager.filter_on": "1 zolang het filter aanstond, 0 zolang het uitstond. Als reeks, zodat 'wanneer stond dit filter aan' een antwoord heeft en geen gok op basis van wanneer er weer iets weggegooid werd. Regel: Zet het hele filter aan of uit. Staat hij uit, dan doet GEEN ENKEL subfilter iets. Let op: `filter on` terwijl er al een categorale regel klaarstaat, is de klik die dat verkeer stilzet.",
      "pfhelp.meshmanager.filter_passed": "Pakketten die het filter langs alle regels doorliet. Alleen samen met `filter_dropped` bruikbaar: 500 weg is veel bij 600 door en weinig bij 600.000 door. Regel: Zet het hele filter aan of uit. Staat hij uit, dan doet GEEN ENKEL subfilter iets. Let op: `filter on` terwijl er al een categorale regel klaarstaat, is de klik die dat verkeer stilzet.",
      "pfhelp.meshmanager.filter_rate_capped": "GEEN dropteller. Het aantal vensters waarin de limiet werkelijk BEET. Deel dit door `filter_rate_windows` en u hebt het cijfer waarmee u een limiet bijstelt: 12 van 4000 vensters is een limiet die af en toe een uitschieter afvlakt en verder niets doet; 3900 van 4000 is een limiet die de normale gang van zaken is geworden en dus te laag staat -- terwijl het aantal weggegooide pakketten in beide gevallen hetzelfde kan zijn. Dáárom staan deze twee naast de dropteller en niet in plaats van hem. Regel: Gooit pakketten van dat type weg boven <limit> stuks per <secs> seconden.",
      "pfhelp.meshmanager.filter_rate_windows": "GEEN dropteller. Het aantal tijdvensters waarin er verkeer van dit soort langskwam -- dus hoe vaak de limiet de KANS had om te bijten. De noemer van de verhouding hieronder. Regel: Gooit pakketten van dat type weg boven <limit> stuks per <secs> seconden.",
      "pfrule.meshcore_filter.filter_drop_channel": "kanaallijst",
      "pfrule.meshcore_filter.filter_drop_hash": "minimale padhashlengte",
      "pfrule.meshcore_filter.filter_drop_hops": "hoplimiet per pakkettype",
      "pfrule.meshcore_filter.filter_drop_malformed": "controle op misvormde tekst",
      "pfrule.meshcore_filter.filter_drop_rate": "snelheidslimiet per pakkettype",
      "pfrule.meshcore_filter.filter_drop_type": "(bestaat niet in deze variant)",
      "pfrule.meshcore_filter.filter_dropped": "hoofdschakelaar van het filter",
      "pfrule.meshcore_filter.filter_exempt": "(geen vrijstellingslijst in deze variant)",
      "pfrule.meshcore_filter.filter_on": "hoofdschakelaar van het filter",
      "pfrule.meshcore_filter.filter_passed": "hoofdschakelaar van het filter",
      "pfrule.meshcore_filter.filter_rate_capped": "snelheidslimiet per pakkettype",
      "pfrule.meshcore_filter.filter_rate_windows": "snelheidslimiet per pakkettype",
      "pfrule.meshmanager.filter_drop_channel": "geblokkeerde kanalen (zwarte lijst)",
      "pfrule.meshmanager.filter_drop_hash": "minimale padhashlengte",
      "pfrule.meshmanager.filter_drop_hops": "hoplimiet per pakkettype",
      "pfrule.meshmanager.filter_drop_malformed": "controle op misvormde groepstekst",
      "pfrule.meshmanager.filter_drop_rate": "snelheidslimiet per pakkettype",
      "pfrule.meshmanager.filter_drop_type": "pakkettype helemaal dicht",
      "pfrule.meshmanager.filter_dropped": "hoofdschakelaar van het filter",
      "pfrule.meshmanager.filter_exempt": "vrijstellingslijst (ACL)",
      "pfrule.meshmanager.filter_on": "hoofdschakelaar van het filter",
      "pfrule.meshmanager.filter_passed": "hoofdschakelaar van het filter",
      "pfrule.meshmanager.filter_rate_capped": "snelheidslimiet per pakkettype",
      "pfrule.meshmanager.filter_rate_windows": "snelheidslimiet per pakkettype",
      "pfwarn.meshcore_filter.filter_drop_channel": "De lijst staat niet in het opgeslagen blok; alleen `filter channel list` op de node zelf vertelt wat er nu doorgelaten wordt.",
      "pfwarn.meshcore_filter.filter_drop_hash": "Deze regel raakt het verkeer dat deze node voor ANDEREN zou doorsturen. Zet u hem strenger, dan verspreidt dit knooppunt minder van het mesh -- adverts en padopbouw van andere nodes vallen daar ook onder. Kijk dus niet alleen naar wat u wegknipt maar ook naar wat u niet meer doorgeeft.",
      "pfwarn.meshcore_filter.filter_drop_hops": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfwarn.meshcore_filter.filter_drop_rate": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfwarn.meshcore_filter.filter_dropped": "De filterconfiguratie van deze variant overleefde in de praktijk een reboot NIET. Controleer met `filter count` ná een herstart of uw regels er nog staan.",
      "pfwarn.meshcore_filter.filter_on": "De filterconfiguratie van deze variant overleefde in de praktijk een reboot NIET. Controleer met `filter count` ná een herstart of uw regels er nog staan.",
      "pfwarn.meshcore_filter.filter_passed": "De filterconfiguratie van deze variant overleefde in de praktijk een reboot NIET. Controleer met `filter count` ná een herstart of uw regels er nog staan.",
      "pfwarn.meshcore_filter.filter_rate_capped": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfwarn.meshcore_filter.filter_rate_windows": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfwarn.meshmanager.filter_drop_hash": "Deze regel raakt het verkeer dat deze node voor ANDEREN zou doorsturen. Zet u hem strenger, dan verspreidt dit knooppunt minder van het mesh -- adverts en padopbouw van andere nodes vallen daar ook onder. Kijk dus niet alleen naar wat u wegknipt maar ook naar wat u niet meer doorgeeft.",
      "pfwarn.meshmanager.filter_drop_hops": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfwarn.meshmanager.filter_drop_rate": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfwarn.meshmanager.filter_drop_type": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfwarn.meshmanager.filter_dropped": "Deze firmware kan het filter na herhaalde herstarts zélf uit laten staan (veilige modus, veld `disarmed`). De regels staan er dan nog en komen bij een schone start terug.",
      "pfwarn.meshmanager.filter_on": "Deze firmware kan het filter na herhaalde herstarts zélf uit laten staan (veilige modus, veld `disarmed`). De regels staan er dan nog en komen bij een schone start terug.",
      "pfwarn.meshmanager.filter_passed": "Deze firmware kan het filter na herhaalde herstarts zélf uit laten staan (veilige modus, veld `disarmed`). De regels staan er dan nog en komen bij een schone start terug.",
      "pfwarn.meshmanager.filter_rate_capped": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfwarn.meshmanager.filter_rate_windows": "Type 00 (REQ) en 01 (RESPONSE) is beheerverkeer. Een regel daarop knipt het beheerverkeer weg dat deze node voor ANDERE nodes zou doorsturen -- dan kan iemand anders zijn eigen repeater niet meer bereiken via dit knooppunt. Uw eigen toegang tot DEZE node loopt er niet langs.",
      "pfvariant.meshcore_filter": "MeshCore met filterpatch",
      "pfvariant.meshmanager": "MeshManager-firmware",
      "state.on": "aan",
      "state.off": "uit",
      "filternote.variant": "Deze node draait firmware met een pakketfilter",
      "filternote.none": "maar heeft er nog geen enkele meting over gemeld.",
      "filternote.cliOnly": "De statistieken van deze variant zijn voor iedereen leesbaar, maar de filtertellers zijn alleen via de admin-CLI over de radio op te halen — en die weg antwoordt niet altijd.",
      "filter.hash": "Hash",
      "filter.hits": "Treffers",
    },
    ...
    en: {
      ...
      "live.pacman": "Pac-Man mode",
      "live.pacman_title": "Same packets, same timings: the dot becomes a hungry disc eating the pellets of its own trail",
      ...
      "block.airtime": "Airtime",
      "block.filter": "Packet filter",
      "block.filterstats": "Filter: breakdown",
      "block.other": "Other",
      "block.charts": "Charts",
      "block.map": "Link map",
      ...
      "metric.filter_exempt": "Exempt (access list)",
      "metric.filter_drop_hops": "Dropped: hop limit",
      "metric.filter_drop_rate": "Dropped: rate limit",
      "metric.filter_drop_type": "Dropped: packet type blocked",
      "metric.filter_drop_hash": "Dropped: path hash too short",
      "metric.filter_drop_channel": "Dropped: blocked channel",
      "metric.filter_drop_malformed": "Dropped: malformed group text",
      "metric.filter_rate_windows": "Rate windows with traffic",
      "metric.filter_rate_capped": "Windows the limit capped",
      "metric.request_successes": "Requests succeeded",
      "metric.request_failures": "Requests failed",
      "metric.out_path": "Outgoing path",

      "filter.trunc": "The node reported that this breakdown did not fit in the message in full. What is shown is correct; it is just not all of it.",
      "filter.perType": "Dropped by packet type",
      "filter.type": "Type",
      "filter.total": "Total",
      "filter.why": "Reason",
      "filter.ratePressure": "Rate-limit pressure",
      "filter.rateHint": "How often the limit actually capped traffic, against how often it had the chance. A low share is a limit with room to spare; a high share is one that structurally cuts traffic away.",
      "filter.windows": "Windows",
      "filter.bit": "Capped",
      "filter.share": "Share",
      "filter.peak": "Peak",
      "filter.limit": "Limit",
      "filter.exempt": "Past the filter via the access list",
      "filter.exemptHint": "Traffic that would have hit the rules and was let past by the access list. If this runs high next to a low ‘Forwarded’, the filter mostly works on traffic that fell outside it anyway.",
      "filter.count": "Count",
      "filter.channels": "Blocked channels",
      "filter.channelsHint": "A repeater never sees a channel name, only one byte: sha256(key)[0]. That byte travels unencrypted in every group message, so it gives away nothing here that is not already on the air — roughly 1 channel in 256 shares it with another.",
      "pfhelp.meshcore_filter.filter_drop_channel": "Channel traffic dropped by the channel list. Check which way that list works on this variant. Rule: Filters group text on the channels in this list. Which way the list works -- pass or block -- is not stated in this build's firmware strings; check with `filter channel list` on the node before relying on it.",
      "pfhelp.meshcore_filter.filter_drop_hash": "Packets dropped because their path hash was shorter than the configured minimum. Rule: Drops packets with a path hash SMALLER than min_bytes.",
      "pfhelp.meshcore_filter.filter_drop_hops": "Packets dropped for having travelled too many hops. Rule: Drops packets of that type which travelled MORE hops than max_hops. Here 0 means NO limit -- not 'drop everything'.",
      "pfhelp.meshcore_filter.filter_drop_malformed": "Packets dropped because their text content was recognised as corrupted. Rule: Drops packets whose text content is recognised as corrupted.",
      "pfhelp.meshcore_filter.filter_drop_rate": "Packets dropped because their type's rate limit was full. Rule: Drops packets of that type above <limit> per <secs> seconds.",
      "pfhelp.meshcore_filter.filter_drop_type": "Packets dropped because their whole packet type is shut. If this climbs, no limit was hit -- a door is closed. This variant cannot shut a packet type completely and therefore counts nothing for it. The strictest option here is `filter rate <type> 1 <secs>` or a low hop limit.",
      "pfhelp.meshcore_filter.filter_dropped": "All dropped packets together: the sum of the per-reason drop counters. The number that shows filtering IS happening; the reason is in the neighbouring tiles. Rule: Turns the whole filter on or off. While off, NO subfilter does anything -- the rules stay but nothing is dropped.",
      "pfhelp.meshcore_filter.filter_exempt": "Packets that skipped the rules entirely because they were exempt. If this climbs while you exempted nothing, this node filters less than you think. This variant has no exemptions and does not report the counter.",
      "pfhelp.meshcore_filter.filter_on": "1 while the filter was on, 0 while it was off. Kept as a series so that 'when was this filter on' has an answer instead of being guessed from when drops resumed. Rule: Turns the whole filter on or off. While off, NO subfilter does anything -- the rules stay but nothing is dropped.",
      "pfhelp.meshcore_filter.filter_passed": "Packets the filter let through past every rule. Only useful next to `filter_dropped`: 500 dropped is a lot against 600 passed and little against 600,000. Rule: Turns the whole filter on or off. While off, NO subfilter does anything -- the rules stay but nothing is dropped.",
      "pfhelp.meshcore_filter.filter_rate_capped": "NOT a drop counter. The number of windows in which the limit actually BIT. Divide this by `filter_rate_windows` and you have the number you tune a limit with: 12 out of 4000 windows is a limit that occasionally shaves a spike and otherwise does nothing; 3900 out of 4000 is a limit that has become the normal state of affairs and is therefore set too low -- while the dropped-packet count can be identical in both cases. That is why these two sit NEXT TO the drop counter rather than instead of it. Rule: Drops packets of that type above <limit> per <secs> seconds.",
      "pfhelp.meshcore_filter.filter_rate_windows": "NOT a drop counter. The number of time windows in which such traffic passed -- i.e. how often the limit had the CHANCE to bite. The denominator of the ratio below. Rule: Drops packets of that type above <limit> per <secs> seconds.",
      "pfhelp.meshmanager.filter_drop_channel": "Channel traffic dropped by the channel list. Check which way that list works on this variant. Rule: `channel add` BLOCKS that channel for group text; anything not listed passes. The opposite of the stock variant.",
      "pfhelp.meshmanager.filter_drop_hash": "Packets dropped because their path hash was shorter than the configured minimum. Rule: Only forwards packets with a path hash of at least min_bytes bytes.",
      "pfhelp.meshmanager.filter_drop_hops": "Packets dropped for having travelled too many hops. Rule: Lets that packet type travel at most max_hops hops. Here 0 means: stop forwarding this type ENTIRELY -- the opposite of the stock variant.",
      "pfhelp.meshmanager.filter_drop_malformed": "Packets dropped because their text content was recognised as corrupted. Rule: Drops group text with an impossible structure.",
      "pfhelp.meshmanager.filter_drop_rate": "Packets dropped because their type's rate limit was full. Rule: Drops packets of that type above <limit> per <secs> seconds.",
      "pfhelp.meshmanager.filter_drop_type": "Packets dropped because their whole packet type is shut. If this climbs, no limit was hit -- a door is closed. Rule: `off` stops forwarding that packet type ENTIRELY -- not a limit but a closed door. The heaviest filter action this firmware offers.",
      "pfhelp.meshmanager.filter_dropped": "All dropped packets together: the sum of the per-reason drop counters. The number that shows filtering IS happening; the reason is in the neighbouring tiles. Rule: Turns the whole filter on or off. While off, NO subfilter does anything. Note: `filter on` while a blanket rule is already staged is the click that silences that traffic.",
      "pfhelp.meshmanager.filter_exempt": "Packets that skipped the rules entirely because they were exempt. If this climbs while you exempted nothing, this node filters less than you think. This firmware DOES report the exemption counter, but this codebase contains no filter command that sets the exemption list. What fills the list cannot be substantiated here, so no syntax is given.",
      "pfhelp.meshmanager.filter_on": "1 while the filter was on, 0 while it was off. Kept as a series so that 'when was this filter on' has an answer instead of being guessed from when drops resumed. Rule: Turns the whole filter on or off. While off, NO subfilter does anything. Note: `filter on` while a blanket rule is already staged is the click that silences that traffic.",
      "pfhelp.meshmanager.filter_passed": "Packets the filter let through past every rule. Only useful next to `filter_dropped`: 500 dropped is a lot against 600 passed and little against 600,000. Rule: Turns the whole filter on or off. While off, NO subfilter does anything. Note: `filter on` while a blanket rule is already staged is the click that silences that traffic.",
      "pfhelp.meshmanager.filter_rate_capped": "NOT a drop counter. The number of windows in which the limit actually BIT. Divide this by `filter_rate_windows` and you have the number you tune a limit with: 12 out of 4000 windows is a limit that occasionally shaves a spike and otherwise does nothing; 3900 out of 4000 is a limit that has become the normal state of affairs and is therefore set too low -- while the dropped-packet count can be identical in both cases. That is why these two sit NEXT TO the drop counter rather than instead of it. Rule: Drops packets of that type above <limit> per <secs> seconds.",
      "pfhelp.meshmanager.filter_rate_windows": "NOT a drop counter. The number of time windows in which such traffic passed -- i.e. how often the limit had the CHANCE to bite. The denominator of the ratio below. Rule: Drops packets of that type above <limit> per <secs> seconds.",
      "pfrule.meshcore_filter.filter_drop_channel": "channel list",
      "pfrule.meshcore_filter.filter_drop_hash": "minimum path hash length",
      "pfrule.meshcore_filter.filter_drop_hops": "hop limit per packet type",
      "pfrule.meshcore_filter.filter_drop_malformed": "malformed text check",
      "pfrule.meshcore_filter.filter_drop_rate": "rate limit per packet type",
      "pfrule.meshcore_filter.filter_drop_type": "(does not exist in this variant)",
      "pfrule.meshcore_filter.filter_dropped": "filter master switch",
      "pfrule.meshcore_filter.filter_exempt": "(no exemption list in this variant)",
      "pfrule.meshcore_filter.filter_on": "filter master switch",
      "pfrule.meshcore_filter.filter_passed": "filter master switch",
      "pfrule.meshcore_filter.filter_rate_capped": "rate limit per packet type",
      "pfrule.meshcore_filter.filter_rate_windows": "rate limit per packet type",
      "pfrule.meshmanager.filter_drop_channel": "blocked channels (block-list)",
      "pfrule.meshmanager.filter_drop_hash": "minimum path hash length",
      "pfrule.meshmanager.filter_drop_hops": "hop limit per packet type",
      "pfrule.meshmanager.filter_drop_malformed": "malformed group text check",
      "pfrule.meshmanager.filter_drop_rate": "rate limit per packet type",
      "pfrule.meshmanager.filter_drop_type": "packet type fully closed",
      "pfrule.meshmanager.filter_dropped": "filter master switch",
      "pfrule.meshmanager.filter_exempt": "exemption list (ACL)",
      "pfrule.meshmanager.filter_on": "filter master switch",
      "pfrule.meshmanager.filter_passed": "filter master switch",
      "pfrule.meshmanager.filter_rate_capped": "rate limit per packet type",
      "pfrule.meshmanager.filter_rate_windows": "rate limit per packet type",
      "pfwarn.meshcore_filter.filter_drop_channel": "The list is not in the stored blob; only `filter channel list` on the node itself tells you what currently passes.",
      "pfwarn.meshcore_filter.filter_drop_hash": "This rule affects traffic this node would forward for OTHERS. Tighten it and this hop spreads less of the mesh -- other nodes' adverts and path discovery included. So watch not only what you cut away, but also what you stop passing on.",
      "pfwarn.meshcore_filter.filter_drop_hops": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfwarn.meshcore_filter.filter_drop_rate": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfwarn.meshcore_filter.filter_dropped": "In practice this variant's filter configuration did NOT survive a reboot. Verify with `filter count` after a restart that your rules are still there.",
      "pfwarn.meshcore_filter.filter_on": "In practice this variant's filter configuration did NOT survive a reboot. Verify with `filter count` after a restart that your rules are still there.",
      "pfwarn.meshcore_filter.filter_passed": "In practice this variant's filter configuration did NOT survive a reboot. Verify with `filter count` after a restart that your rules are still there.",
      "pfwarn.meshcore_filter.filter_rate_capped": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfwarn.meshcore_filter.filter_rate_windows": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfwarn.meshmanager.filter_drop_hash": "This rule affects traffic this node would forward for OTHERS. Tighten it and this hop spreads less of the mesh -- other nodes' adverts and path discovery included. So watch not only what you cut away, but also what you stop passing on.",
      "pfwarn.meshmanager.filter_drop_hops": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfwarn.meshmanager.filter_drop_rate": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfwarn.meshmanager.filter_drop_type": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfwarn.meshmanager.filter_dropped": "This firmware may leave the filter off by itself after repeated restarts (safe mode, field `disarmed`). The rules remain and return after a clean start.",
      "pfwarn.meshmanager.filter_on": "This firmware may leave the filter off by itself after repeated restarts (safe mode, field `disarmed`). The rules remain and return after a clean start.",
      "pfwarn.meshmanager.filter_passed": "This firmware may leave the filter off by itself after repeated restarts (safe mode, field `disarmed`). The rules remain and return after a clean start.",
      "pfwarn.meshmanager.filter_rate_capped": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfwarn.meshmanager.filter_rate_windows": "Types 00 (REQ) and 01 (RESPONSE) are admin traffic. A rule on those cuts the admin traffic this node would forward for OTHER nodes -- someone else may then be unable to reach their own repeater through this hop. Your own access to THIS node does not pass through the filter.",
      "pfvariant.meshcore_filter": "MeshCore with filter patch",
      "pfvariant.meshmanager": "MeshManager firmware",
      "state.on": "on",
      "state.off": "off",
      "filternote.variant": "This node runs firmware with a packet filter",
      "filternote.none": "but has never reported a single filter figure.",
      "filternote.cliOnly": "This variant’s statistics are readable by anyone, but its filter counters can only be fetched over the radio through the admin CLI — and that path does not always answer.",
      "filter.hash": "Hash",
      "filter.hits": "Hits",
    },
  };
