...
from . import (audit, auth, clocksync, commanding, companions, compare, config,
               db, discovery, firmware, hadiscovery, metrics, monitors,
               mqtt_ingest, nodeconfig, nodecred, pktfilter, qrsvg, ratelimit,
               rbac, retention, rooms, sensornode, sensorpush, snmp, sweepsched,
               tsdb)
...
def _keuze_id(waarde) -> int | None:
    ...
    De enkelvoudige tegenhanger van ``_keuzes``: een dropdown met "— geen —" stuurt
    een lege string, en dat hoort None te worden en geen 500 op int("")."""
    tekst = str(waarde or "").strip()
...
# ==================== Companions ==============================================
#
# Twee tabbladen die op de companions leunen en tegelijk generiek blijven:
#
#   /admin/companions        de beheerde companions -- lijst + CRUD (voor
#                            tientallen gebouwd), en per companion een eigen
#                            pagina met de commando-knoppen én de seriële CLI.
#   /admin/senddm            een GENERIEKE Send-DM-tab: kies een afzender-node,
#                            kies een bestemming (companion of een contact van die
#                            node), typ een tekst, verstuur.
#
# De grenzen: de LIJST muteren is een serverhandeling (``server.companions``); een
# DM VERSTUREN is een node-handeling op de afzender (``node.instelling.merkbaar``),
# want dat kost zendtijd op die node. Het vervoer zit in ``companions.send_dm`` en
# nergens hier -- de routes geven een afzender-node en een tekst.
...
def companions_page(request: Request):
    """Wereld 1, een eigen tab: de beheerde companions.
...
def companion_cmd(request: Request, cid: int, cmd: str = Form(""),
                  ...
                  level: str = Form(""), sev: str = Form(""), rtttl: str = Form(""),
                  range: str = Form(""), mode: str = Form(""), sub: str = Form(""),
                  value: str = Form(""), slot: str = Form(""), text: str = Form(""),
                  csrf: str = Form(...)):
    ...
    user = require_perm(request, "node.instelling.merkbaar", rep)
    check_csrf(request, csrf)
    args = {"state": state, "level": level, "sev": sev, "rtttl": rtttl,
            "range": range, "mode": mode, "sub": sub, "value": value,
            "slot": slot, "text": text}
